module Api
  module V1
    class DailyFrequenciesController < Api::V1::BaseController
      respond_to :json
      ...
      def process_one
        ...
        if @daily_frequency.valid?
          @daily_frequency = find_or_create_daily_frequency(frequency_params)
      ...
      def process_multiple
        frequency_params = daily_frequency_params.merge(
        ...
        if @daily_frequency.valid?
          ...
          @class_numbers.each do |class_number|
            ...
            @daily_frequencies << find_or_create_daily_frequency(frequency_params)
          end
          ...
          @student_enrollments.each do |student_enrollment|
            ...
            @students << {
              ...
              daily_frequencies: @daily_frequencies.map { |daily_frequency|
                (
                  daily_frequency.students.where(student_id: student.id).first ||
                  daily_frequency.students.create(
                    student_id: student.id,
                    dependence: student_has_dependence?(
                      student_enrollment.id,
                      @daily_frequencies[0].discipline_id
                    ),
                    present: true,
                    active: true
                  )
