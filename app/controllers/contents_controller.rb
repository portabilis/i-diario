class ContentsController < ApplicationController
  ...
  def index
    ...
    if params[:fetch_for_discipline_records]
      ...
      @contents = apply_scopes(Content).by_teacher(current_teacher.id)
    elsif params[:filter][:start_with_description]
      @contents = Content.by_teacher(current_teacher.id).start_with_description(params[:filter][:start_with_description])
    end
