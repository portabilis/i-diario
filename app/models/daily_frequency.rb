class DailyFrequency < ActiveRecord::Base
  ...
  scope :by_frequency_date, ->(frequency_date) { where(frequency_date: frequency_date.to_date) }
  scope :by_frequency_date_between, ->(start_at, end_at){ where(frequency_date: start_at.to_date..end_at.to_date) }
  scope :by_class_number, ->(class_number) { where(class_number: class_number) }
  scope :by_school_calendar_id, ->(school_calendar_id) { where(school_calendar_id: school_calendar_id) }
  scope :general_frequency, -> { where(discipline_id: nil, class_number: nil) }
  scope :has_frequency_for_student, ->(student_id) { joins(:students).merge(DailyFrequencyStudent.by_student_id(student_id)) }
  ...
  def build_or_find_by_student(student)
    find_by_student(student.id) || students.build(student_id: student.id, present: 1)
  end
  ...
  def frequency_date_must_be_less_than_or_equal_to_today
    ...
    errors.add(:frequency_date, :must_be_less_than_or_equal_to_today) if frequency_date > Time.zone.today
  end
