class AttendanceRecordReport < BaseReport
  ...
  def daily_frequencies_table
    ...
    sliced_frequencies_and_events.each_with_index do |frequencies_and_events_slice, index|
      ...
      sliced_students_cells.each_with_index do |students_cells_slice, i|
        data = [
        ...
        if i == sliced_students_cells.count - 1 && index == sliced_frequencies_and_events.count - 1
          students_cells_slice << [{content: "Aulas dadas: #{frequencies_and_events.count}", colspan: 44, align: :center}]
        end
        ...
        start_new_page if i < sliced_students_cells.count - 1
      end
