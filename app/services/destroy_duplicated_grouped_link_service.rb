class DestroyDuplicatedGroupedLinkService
  def self.call
    ActiveRecord::Base.transaction do
      destroy_duplicated_groupers
      destroy_orphaned_groupers
    end
  end

  # Remove groupers duplicados (mais de 1 grouper ativo para a mesma area de conhecimento)
  def self.destroy_duplicated_groupers
  ...
  # Remove groupers orfaos (grouper ativo sem disciplinas regulares ativas na mesma turma/area)
  def self.destroy_orphaned_groupers
    ids = GroupedTeacherDisciplineClassrooms.pluck(:link_id)
    TeacherDisciplineClassroom.where(id: ids).find_each(&:destroy)
  end
end
