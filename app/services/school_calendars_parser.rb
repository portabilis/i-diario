    ...
    existing_school_calendars_from_api.each do |school_calendar_from_api|
      ...
      steps_from_classrooms.each_with_index do |classroom_step, classroom_index|
        if SchoolCalendarClassroom.by_classroom_api_code(classroom_step['turma_id']).first
          classroom = school_calendar.classrooms.by_classroom_api_code(classroom_step['turma_id']).first
          classroom_step['etapas'].each_with_index do |step, step_index|
            if classroom.classroom_steps.any?
              update_classrooms_step_start_at(classroom, step_index, step)
              update_classrooms_step_end_at(classroom, step_index, step)
            else
