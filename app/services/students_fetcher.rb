class StudentsFetcher
  def initialize(
    ...
    step = nil,
    remove_exempted_from_discipline = false
  )
    @classroom = classroom
  ...
  def fetch
    ...
    students = Student.where(id: student_ids).order_by_sequence(@classroom, @step.start_at, @step.end_at)

    if @discipline.present? && @step.present?
