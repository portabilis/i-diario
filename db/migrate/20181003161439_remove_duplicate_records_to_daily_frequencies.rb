class RemoveDuplicateRecordsToDailyFrequencies < ActiveRecord::Migration
  def change
    execute <<-SQL
      DO $$DECLARE
      ...
      BEGIN
        ...
        DELETE FROM daily_frequency_students WHERE daily_frequency_id = daily_frequency.id;
        DELETE FROM daily_frequencies WHERE id = daily_frequency.id;
        ELSE
