class RebalanceLessonPlansIndexes < ActiveRecord::Migration[5.0]
  ...
  def up
    ...
    remove_index :lesson_plans, column: :school_calendar_id, algorithm: :concurrently
  end

  def down
    add_index :lesson_plans, :school_calendar_id, algorithm: :concurrently
    remove_index :lesson_plans, column: [:classroom_id, :start_at], algorithm: :concurrently
    remove_index :lesson_plans, column: :teacher_id, algorithm: :concurrently
  end
end
