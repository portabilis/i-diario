        ...
        function generateMaskSet(opts, multi) {
            ...
            function analyseMask(mask) {
                ...
                function maskToken(is***REMOVED***, isOptional, isQuantifier, isAlternator) {
                    this.matches = [];
                    this.is***REMOVED*** = is***REMOVED*** || false;
                            ...
                            if (openenings.length > 0) {
                                ...
                                if (currentOpeningToken.isAlternator) {  //handle alternator (a) | (b) case
                                    ...
                                    for (var mndx = 0; mndx < alternator.matches.length; mndx++) {
                                        alternator.matches[mndx].is***REMOVED*** = false; //don't mark alternate ***REMOVED*** as group
                                    }
                                    if (openenings.length > 0) {
                                        currentOpeningToken = openenings[openenings.length - 1];
                        ...
                        default:
                            if (openenings.length > 0) {
                                ...
                                if (currentOpeningToken.matches.length > 0) {
                                    ...
                                    if (lastMatch["is***REMOVED***"]) { //this is not a group but a normal mask => convert
                                        lastMatch.is***REMOVED*** = false;
                ...
                function ResolveTestFromToken(maskToken, ndxInitializer, loopNdx, quantifierRecurse) { //ndxInitilizer contains a set of indexes to speedup searches in the mtokens
                    function handleMatch(match, loopNdx, quantifierRecurse) {
                        ...
                        if (testPos == pos && match.matches == undefined) {
                            ...
                            if (match.is***REMOVED*** && quantifierRecurse !== true) { //when a group pass along to the quantifier
                                ...
                                for (var qndx = (ndxInitializer.length > 0 && quantifierRecurse !== true) ? ndxInitializer.shift() : 0; (qndx < (isNaN(qt.quantifier.max) ? qndx + 1 : qt.quantifier.max)) && testPos <= pos; qndx++) {
                                    ...
                                    match = handleMatch(token***REMOVED***, [qndx].concat(loopNdx), true);
                                    if (match) {
                                        //get latest match
                                        var latestMatch = matches[matches.length - 1]["match"];
                                        latestMatch.optionalQuantifier = qndx > (qt.quantifier.min - 1);
                                        var isFirstMatch = $.inArray(latestMatch, token***REMOVED***.matches) == 0;
